...
export const OkrInputForm = ({
  ...
  return (
      ...
      <div
        className="px-6 top-4 sticky max-w-screen-sm py-6 border-2 border-gray-100 h-fit bg-gray-50 rounded-lg shadow-md flex flex-col gap-4">
        ...
        {keyResults.map((keyResult, index) => (
          <div
            key={index}
            className={`flex flex-col gap-4 my-4 border px-10 py-6 ${index % 2 === 0 ? `bg-blue-100` : `bg-blue-200`} rounded-lg shadow-sm`}
          >
            <input
              className="w-full py-3 px-4 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white shadow-sm"
              placeholder="Title"
